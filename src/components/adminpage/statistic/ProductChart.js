  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let response;
        if (view === "overview") {
          response = await fetchWithAuth(summaryApi.getTop5BestSellingProduct.url, {
            method: summaryApi.getTop5BestSellingProduct.method,
          });
        } else {
          const startDate = moment().year(selectedYear).month(selectedMonth).startOf('month').format('YYYY-MM-DD');
          const endDate = moment().year(selectedYear).month(selectedMonth).endOf('month').format('YYYY-MM-DD');
